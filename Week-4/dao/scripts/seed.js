...
async function main() {
    ...
    transaction = await funder.sendTransaction({ to: dao.address, value: value }); // 1000 Ether
    await transaction.wait();
    console.log(`Sent ${value}ETH to DAO treasury...\n`);

    for (var no = 1; no <= 3; no++) {
        // Create proposal
        transaction = await dao.connect(investor1).createProposal(`Proposal ${no}`, tokens(100), recipient.address);
        result = await transaction.wait();

        // Vote 1
        transaction = await dao.connect(investor1).vote(no);
        result = await transaction.wait();

        // Vote 2
        transaction = await dao.connect(investor2).vote(no);
        result = await transaction.wait();

        // Vote 3
        transaction = await dao.connect(investor3).vote(no);
        result = await transaction.wait();

        // Finalize
        transaction = await dao.connect(investor1).finalizeProposal(no);
        result = await transaction.wait();

        console.log(`Created & Finalized Proposal ${no}\n`);
    }
